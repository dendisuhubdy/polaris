...
func (cv *ContainerValidation) validateSecurity(conf *config.Configuration, controllerName string) {
	...
	if config.IsActionable(conf, conf.Security, name, controllerName) {
		id := config.GetIDFromField(conf.Security, name)
		hasSecurityError :=
			!cv.validateCapabilities(id, conf.Security.Capabilities.Error, config.SeverityError)
		hasSecurityWarning :=
			!cv.validateCapabilities(id, conf.Security.Capabilities.Warning, config.SeverityWarning)
		hasSecurityCheck := func(confLists config.SecurityCapabilityLists) bool {
		...
		if !hasSecurityError && !hasSecurityWarning &&
			(hasSecurityCheck(conf.Security.Capabilities.Error) ||
				hasSecurityCheck(conf.Security.Capabilities.Warning)) {
			cv.addSuccess(messages.SecurityCapabilitiesSuccess, category, id)
		}
	}
}

func (cv *ContainerValidation) validateCapabilities(id string, confLists config.SecurityCapabilityLists, severity config.Severity) bool {
	category := messages.CategorySecurity
